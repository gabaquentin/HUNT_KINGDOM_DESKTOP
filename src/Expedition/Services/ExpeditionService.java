...
public class ExpeditionService implements IService<Expedition> {
    ...
    public List<Expedition> afficher() {
        ...
        try {
            ...
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                Expedition E = new Expedition(rs.getInt(1), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getString(10));
                list.add(E);
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
